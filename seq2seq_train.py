...
file_path = './dulieu/tt_ca_nhan/data/'

with open(file_path + 'from.txt', 'r', encoding='utf-8') as fopen:
    text_from = fopen.read().lower()
    text_from = re.sub('[“”!@#$]', '', text_from)
    text_from = text_from.split('\n')
with open(file_path + 'to.txt', 'r', encoding='utf-8') as fopen:
    text_to = fopen.read().lower()
    text_to = re.sub('[“”!@#$]', '', text_to)
    text_to = text_to.split('\n')

concat_from = []
for i in text_from:
    k = ViTokenizer.tokenize(i).split(' ')
    concat_from = np.append(concat_from, k)
...
for i in text_to:
    k = ViTokenizer.tokenize(i).split(' ')
    concat_to = np.append(concat_to, k)
...
checkpoint_dir = os.path.abspath(os.path.join('./dulieu/tt_ca_nhan/', "checkpoints_chatbot"))
checkpoint_prefix = os.path.join(checkpoint_dir, "model_tt_ca_nhan")
...
def predict(sentence):
    X_in = []
    m = ViTokenizer.tokenize(sentence).split(' ')
    print(m)
    for word in m:
        try:
...
checkpoint_file = tf.train.latest_checkpoint(os.path.join('./dulieu/tt_ca_nhan/', 'checkpoints_chatbot'))
